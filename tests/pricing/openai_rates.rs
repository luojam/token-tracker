...
#[test]
fn bundled_rates_switch_at_the_request_input_boundary() {
    ...
    for (model, tier, short, long) in [
        ...
        (
            ...
            Some([4_000_000, 400_000, 5_000_000, 15_000_000]),
        ),
        (
            "gpt-6-sol",
            Fast,
        ...
        (
            ...
            Some([200_000, 20_000, 250_000, 750_000]),
        ),
        (
            "gpt-6-luna",
            Fast,
            [200_000, 20_000, 250_000, 1_000_000],
            Some([400_000, 40_000, 500_000, 1_500_000]),
        ),
        (
            "gpt-5.6-sol",
            Standard,
        ...
        for (input, expected) in [(272_000, Some(short)), (272_001, long)] {
...
fn unknown_names_and_tiers_never_fall_back_to_supported_rates() {
    ...
    for (provider, model, tier, reason) in [
        ...
        assert_eq!(lookup_rates(&attribution, &tier, 100), Err(reason));
    }
}

#[test]
fn supported_aliases_resolve_to_the_same_rates() {
    for (alias, model) in [
        ("gpt-5.6", "gpt-5.6-sol"),
        ("gpt-5.5-2026-04-23", "gpt-5.5"),
        ("gpt-5.4-mini-2026-03-17", "gpt-5.4-mini"),
    ] {
        let attribution = |model: &str| ModelAttribution {
            provider: "openai".into(),
            model: model.into(),
        };
        assert_eq!(
            lookup_rates(&attribution(alias), &ServiceTier::Standard, 100),
            lookup_rates(&attribution(model), &ServiceTier::Standard, 100)
        );
    }
}

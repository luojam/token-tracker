...
fn imports_append_and_correct_usage_while_retaining_omitted_history() {
    ...
    let mut inputs: Vec<_> = snapshot
        .observations
        .iter()
        .map(|o| o.event.tokens.input)
        .collect();
    inputs.sort();
    assert_eq!(inputs, vec![20, 999_999]);
...
#[test]
fn notices_persist_until_a_successful_replacement() {
    let tree = TempTree::new();

...
#[test]
fn billing_round_trips_and_tracks_usage_corrections() {
...
fn normalization_failures_and_omissions_preserve_billing() {
    ...
    normalized.normalization_version = 2.try_into().unwrap();
    ...
    assert!(matches!(
        store.commit_import(&validated(&normalized)),
    ...
    store.commit_import(&validated(&normalized)).unwrap();
    assert_eq!(store.usage_snapshot().unwrap(), before);
}
...
fn corrupt_billing_and_out_of_range_counts_are_rejected() {
    ...
    let mut store = SqliteUsageStore::open_in_memory().unwrap();
    assert!(matches!(
        store.commit_import(&validated(&session_import(
            "/sessions/huge.jsonl",
            u64::MAX
        ))),
        Err(SqliteStoreError::ValueOutOfRange(_))
